...
public class MyFileAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		View fileView = mInflater.inflate(R.layout.file_manager_item, null);
		...
		CheckBox fileSelected = (CheckBox) fileView
		...
		if (file.isDirectory) {
		...
		} else {// 根据文件的扩展名设置文件的图标
			String ext = fName.substring(fName.lastIndexOf(".") + 1);
			if (file.bitmap != null) {
				fileIcon.setImageBitmap(file.bitmap);
			} else if (exts.containsKey(ext)) {
			...
			Boolean isFileSelected = Constant.fileSelectedState.get(Integer
					.valueOf(position));
			if (null != isFileSelected)
				fileSelected.setChecked(isFileSelected);
		}
